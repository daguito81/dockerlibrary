...
TUTORIAL_ROOT_PATH = os.getenv('TUTORIAL_ROOT_PATH')
...
def load_files_into_db(ds, **kwargs):
    ...
    with engine.connect() as conn:
        ...
        df_npi_small = pd.read_csv(os.path.join(TUTORIAL_ROOT_PATH, "data", "npi_small.csv"))
        ...
        df_state_abbreviations = pd.read_csv(os.path.join(TUTORIAL_ROOT_PATH, "data", "state_abbreviations.csv"))
...
task_transform_data_in_db = BashOperator(
    ...
    bash_command='dbt run --project-dir {}'.format(os.path.join(TUTORIAL_ROOT_PATH, 'dbt')),
    dag=dag)
